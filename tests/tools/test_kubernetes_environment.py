...
internal JSON env var, and the pod shape is ONE REQUIRED ``spec`` posted to the
API server VERBATIM — no default base, no merge. Validating and constraining
...
def _starter_object():
    ...
    return yaml.safe_load((root / "k8s" / "session-pod-template.yaml").read_text())
...
def _kcfg(**overrides):
    """Config with a COMPLETE spec, because the backend requires one.
...
def test_kind_is_the_provisioner_seam():
    """There was a `provisioner: pod` enum. It named the same thing twice: an
    operator writing `kind: Pod` has already said which provisioner they want.
    Dispatching on (apiVersion, kind) means a second kind — a sandbox CRD, say
...
def test_hard_cut_keys_are_not_in_the_schema():
    ...
    assert set(DEFAULT_KUBERNETES_CONFIG) == {
        ...
        "ready_timeout_seconds", "owner_reference",
    ...
    assert not ((gone | {"pod_template", "provisioner", "trusted_sandbox",
...
def test_the_starter_template_is_a_sane_starting_point():
    ...
    # vanilla cluster has. Defaulting to the no-perms SA that k8s/rbac.yaml
...
def test_factory_rejects_a_kind_it_has_no_provisioner_for(monkeypatch, bad):
    ...
    with pytest.raises(ValueError, match="no provisioner for"):
...
def test_kubernetes_skips_the_dangerous_command_guards():
    """There was a `trusted_sandbox` key: the guards stayed ON until the
    operator declared the pods disposable. It made this the only config-gated
    approval bypass in the codebase, while docker, singularity, modal, daytona
    and vercel_sandbox all bypass unconditionally by env_type. Commands run in
    a session pod, not on the host, so the same reasoning applies and the key
    was asking operators to opt in to the default behaviour of every peer."""
    from tools.approval import _should_skip_container_guards

    assert _should_skip_container_guards("kubernetes") is True
    # has_host_access is a docker concept (bind-mounted host paths); it does
    # not gate this backend in either direction.
    assert _should_skip_container_guards("kubernetes", has_host_access=True) is True


def test_kubernetes_is_not_routed_through_the_docker_host_access_check():
    """`_docker_has_host_access` used to special-case kubernetes and dispatch
    to a kubernetes-specific evaluator. With the key gone there is nothing to
    evaluate, and a stray True here would silently re-enable the guards."""
...
def test_yaml_boolean_off_disables_owner_references():
    """k8s/README.md tells operators to write `owner_reference: off`, and YAML
...
# ---------------------------------------------------------------------------
# `spec` is REQUIRED and is posted verbatim
# ---------------------------------------------------------------------------
#
# This replaced a default base + RFC 7386 merge (with a containers-by-name
# exception). The rule was correct and still meant an operator could not
# predict their own pod without knowing an invisible base and simulating a
# merge in their head. These tests pin the replacement: what you write is what
...
def test_a_missing_spec_is_a_clear_error():
    """There is no default pod. Silently inventing one is what created the
    predictability problem in the first place, so the failure names the file to
    copy instead. metadata alone is not a pod."""
    for absent in ({}, {"metadata": {"labels": {"a": "b"}}}, None, "", []):
        with pytest.raises(ValueError, match="spec is required"):
            render_session_object(_kcfg(pod_template_raw=absent))
...
def test_the_starter_constant_matches_the_shipped_yaml():
    """Two copies of the same pod: STARTER_SESSION_OBJECT is what `hermes setup`
    writes into config.yaml, k8s/session-pod-template.yaml is the annotated one
    an operator copies by hand. Drift means two people following two documented
    paths get different pods."""
    import yaml

    from tools.environments.kubernetes import STARTER_SESSION_OBJECT

    root = Path(__file__).resolve().parents[2]
    shipped = yaml.safe_load((root / "k8s" / "session-pod-template.yaml").read_text())
    assert shipped == STARTER_SESSION_OBJECT


def test_the_shipped_starter_template_renders_a_usable_pod():
    """k8s/session-pod-template.yaml is what the error message tells operators
    ...
    root = Path(__file__).resolve().parents[2]
    template = yaml.safe_load((root / "k8s" / "session-pod-template.yaml").read_text())
...
def test_networkpolicy_and_readme_state_what_the_label_selector_covers():
    ...
    # It must NOT still claim config cannot strip the label. `owned_selector`
    # makes that label configuration: an operator who rebrands it gets pods
    # this policy does not select, with unrestricted egress and nothing
    # logging it. The file an admin `oc apply`s has to say so.
    assert "AFTER the user's" not in text, "stale merge-era claim"
    assert "CONFIGURATION, NOT A GUARANTEE" in text
    vap = (K8S_DIR / "validatingadmissionpolicy.yaml").read_text(encoding="utf-8")
    assert "matchCondition false means none of the rules below run" in vap
...
def test_doctor_probes_the_exec_verb_the_client_actually_issues():
    ...
    assert "create pods/exec" in readme
