...
def test_cli_config_example_documents_the_yaml_block():
    ...
    for dead in ("provisioner", "pod_template", "trusted_sandbox",
...
def test_docs_do_not_promise_a_pod_per_session():
    ...
    for relative in ("README.md", "k8s/README.md",
                     "website/docs/user-guide/configuration.md"):
        text = (REPO_ROOT / relative).read_text(encoding="utf-8")
    ...
    assert "Session scope" in (REPO_ROOT / "k8s" / "README.md").read_text(encoding="utf-8")
