...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # `spec` is REQUIRED and there is no default pod, so seed a working
        # object rather than writing a config that fails on first use.
        ...
        print_info(
            ...
            "manifest, and `spec` is posted verbatim with no default base "
            "and no merge. Edit it in config.yaml to set the image, "
            "resources, runtimeClassName for kata, nodeSelector or volumes."
        )
        print_info(
            "  k8s/session-pod-template.yaml is the same object annotated "
