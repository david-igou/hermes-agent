...
- "kubernetes": Execute in a session pod in a Kubernetes/OpenShift cluster
...
def _get_env_config() -> Dict[str, Any]:
    ...
    if env_type == "local":
        ...
        # From the exec container's workingDir in the spec — one source of
        # truth for where a session starts, instead of a config key that could
        # disagree with the pod.
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # The ONE decision Hermes makes in-process, because nothing
        ...
        # which is where an operator goes to be told about a smell.
